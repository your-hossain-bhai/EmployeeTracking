...
//
// This file serves as the entry point for the Android application and sets up
// the platform channels for communication between Flutter and native Android code.
//
// Platform Channels:
// - MethodChannel "com.example.smart_employee/location_control" - Location service control
// - EventChannel "com.example.smart_employee/location_stream" - Location updates stream
// - MethodChannel "com.example.smart_employee/geofence_control" - Geofence management
// - EventChannel "com.example.smart_employee/geofence_stream" - Geofence events stream
//
// NOTE: This implementation uses Flutter embedding v2 for compatibility.
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, LOCATION_CONTROL_CHANNEL)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    ...
                    "pauseService" -> {
                        pauseLocationService(result)
                    }
                    "resumeService" -> {
                        resumeLocationService(result)
                    }
                    "getPermissionStatus" -> {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, GEOFENCE_CONTROL_CHANNEL)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    ...
                    "listGeofences" -> {
                        result.success(GeofenceService.getInstance(this).listGeofences())
                    }
                    else -> {
                        result.notImplemented()
                    }
